...
- 主要原因是部分场景不需要RedisUtil，避免强制引入Redis依赖的问题
- 将各个模块进行抽象，按需引入，有效减少jar包大小

```xml
<!-- 全量引入 -->
<dependency>
  <groupId>cn.katool</groupId>
  <artifactId>katool-all</artifactId>
  <version>${katool.version}</version>
</dependency>

        <!-- 一些基础配件工具，不需要引入其他外部依赖的工具
        包含：OSS存储工具SDK、AuthUtil、图形验证码生成、自定义类加载器、
        AI大模型服务（目前主要是LLM大模型对接，主要对国内的大模型框架进行适配，目前适配了Kimi）-->
<dependency>
<groupId>cn.katool</groupId>
<artifactId>katool-basic</artifactId>
<version>${katool.version}</version>
</dependency>
        <!-- 主要提供RedisUtil工具类-->
<dependency>
<groupId>cn.katool</groupId>
<artifactId>katool-store-redis</artifactId>
<version>${katool.version}</version>
</dependency>

        <!-- 对缓存策略接口、缓存策略工具单独做一个包出来-->
<dependency>
<groupId>cn.katool</groupId>
<artifactId>katool-store-common</artifactId>
<version>${katool.version}</version>
</dependency>
```
...
- ALPHA 2024/08 / 20
  - 历时两个月开发，新增了支持KimiAI的对接
    - 支持Stream流对话以及ask对话、上下文缓存操作、文件操作、ToolCalls等常用方案以及组合方案
    - 支持同一对象多线程请求
    - 支持同一对象采用会话维度进行请求
  - CopyOnTransmittableThreadLocal - 在线程池下的ThreadLocal解决方案（重写了`TransmittableThreadLocal`主要是解决`InheritableThreadLocal`在父子线程中浅拷贝的问题）
  - SessionPackageTheadLocalAdaptor -  对`CopyOnTransmittableThreadLocal `进行包装，解决从会话维度绑定session的问题，这个时候需要自行创建多个对象，建议做关于ai的聊天系统使用这种方案，毕竟ThreadLocal还是会占用space）
  - LocalLockMap - 读写锁映射 锁名-读写锁
  - 模块拆分
...
分布式锁-公平队列后续采用Redis-ZSet实现（之前为了方便直接存在了JVM中，在集群下并不是严格公平，后续会优化）